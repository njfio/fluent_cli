...
async fn demonstrate_mcp_resource_management() -> Result<()> {
    ...
    for resource in &resources {
        ...
        println!(
            ...
            resource.access_permissions.readable, resource.access_permissions.writable
        );
        println!(
            "     Cache policy: cacheable={}, TTL={:?}s",
            resource.cache_policy.cacheable, resource.cache_policy.ttl_seconds
        );
    }
    ...
    println!(
        "📊 Resource statistics available for {} resources",
        all_stats.len()
    );
...
async fn demonstrate_complete_mcp_workflow() -> Result<()> {
    ...
    println!(
        "   Resources: {}",
        resource_manager.list_resources().await.len()
    );
    ...
    println!(
        "   Cache entries: {}",
        cache_stats.get("total_entries").unwrap_or(&json!(0))
    );
    println!(
        "   Cache utilization: {}%",
        cache_stats
    ...
    match resource_manager
        .read_resource("nonexistent://resource")
        .await
    {
