...
async fn main() -> Result<()> {
    ...
    if avg_success_rate > 0.85 && avg_quality > 0.8 {
        ...
        println!(
            "   ✅ Status: GOOD - System performing well with minor optimization opportunities"
        );
    } else {
        println!(
            "   ⚠️  Status: NEEDS IMPROVEMENT - System requires optimization before deployment"
        );
    }
