...
"#;
    ...
    println!("✅ Created tool registry with {} tools", registry.get_all_available_tools().len());
    ...
    assert!(final_content.contains("\"true\""), "Debug should be set to true");
    assert!(final_content.contains("log::info!"), "Should use log::info! instead of println!");
    assert!(!final_content.contains("println!"), "Should not contain any println! statements");
    assert!(final_content.contains("anyhow::Error"), "Should use anyhow::Error in process_data function");
    assert!(final_content.contains("HashMap"), "HashMap should still exist (dry run didn't change it)");
