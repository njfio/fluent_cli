...
async fn demo_replace_first(editor: &StringReplaceEditor, file_path: &std::path::Path) -> Result<()> {
...
async fn demo_specific_occurrence(editor: &StringReplaceEditor, file_path: &std::path::Path) -> Result<()> {
...
async fn demo_error_handling(editor: &StringReplaceEditor, file_path: &std::path::Path) -> Result<()> {
...
mod tests {
    ...
    async fn test_string_replace_integration() {
        ...
        let result = editor.execute_tool("string_replace", &params).await.unwrap();
