...
use fluent_agent::{
    ...
    goal::{Goal, GoalType},
    memory::{SqliteMemoryStore, MemoryItem, MemoryType, MemoryQuery, LongTermMemory},
    ...
    let experiences = vec![
        MemoryItem {
            ...
            metadata: {
                ...
                map.insert("project_type".to_string(), Value::String("rust".to_string()));
                map.insert("outcome".to_string(), Value::String("success".to_string()));
                map.insert("warnings".to_string(), Value::Number(serde_json::Number::from(0)));
                map
            },
            importance: 0.8,
            created_at: Utc::now(),
        ...
        MemoryItem {
            ...
            metadata: {
                ...
                map.insert("concept".to_string(), Value::String("async_programming".to_string()));
                map.insert("importance_level".to_string(), Value::String("high".to_string()));
                map
            },
            importance: 0.9,
            created_at: Utc::now(),
...
async fn demo_context_system() -> Result<()> {
    ...
    let goal = Goal::builder(
        "Demo context management".to_string(),
        GoalType::Analysis
    ).build()?;
    ...
    context.set_variable("project_path".to_string(), "/Users/dev/my_project".to_string());
    context.set_variable("target_language".to_string(), "rust".to_string());
    context.set_variable("compilation_target".to_string(), "x86_64-unknown-linux-gnu".to_string());
