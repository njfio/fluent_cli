...
use anyhow::Result;
use fluent_agent::tools::{ToolRegistry, ToolExecutor};
...
async fn test_tool_registry_integration() -> Result<()> {
    ...
    assert!(registry.is_tool_available("string_replace"), "string_replace tool should be available");
    
    // Get all tools and verify string_replace is included
    let all_tools = registry.get_all_available_tools();
    let string_replace_tool = all_tools.iter()
    ...
    println!("   📝 Tool: {} ({})", string_replace_tool.name, string_replace_tool.description);
    println!("   🏷️  Executor: {}", string_replace_tool.executor);
    
    // Verify other expected tools are also present
    let expected_tools = vec!["read_file", "write_file", "list_directory", "string_replace"];
    for expected_tool in expected_tools {
        assert!(registry.is_tool_available(expected_tool), 
            "Expected tool '{}' should be available", expected_tool);
...
"#;
    ...
    println!("   🔄 First replacement result: {}", 
    ...
    let result = registry.execute_tool("string_replace", &params).await?;
    println!("   🔄 All replacements result: {}", 
        result.lines().take(3).collect::<Vec<_>>().join(" "));
...
"#;
    ...
    let result = registry.execute_tool("string_replace", &params).await?;
    println!("   📏 Line range replacement: {}", 
        result.lines().take(3).collect::<Vec<_>>().join(" "));
    ...
    assert!(content.contains("FIRST println!"), "First occurrence should remain");
    assert!(content.contains("SECOND eprintln!"), "Second occurrence should be changed");
    assert!(content.contains("THIRD println!"), "Third occurrence should remain");
...
async fn test_error_handling() -> Result<()> {
    ...
    let mut params = HashMap::new();
    params.insert("file_path".to_string(), json!(temp_dir.path().join("nonexistent.txt").to_string_lossy()));
    params.insert("old_str".to_string(), json!("test"));
