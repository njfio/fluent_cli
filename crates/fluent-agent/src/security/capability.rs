use super::{
    Capability, Condition, Constraint, Permission, ResourceType, ResourceUsage, SecurityError,
    SecurityPolicy, SecuritySession,
};
use anyhow::Result;
use chrono::Datelike;
...
impl CapabilityManager {
    ...
    pub async fn check_permission(
        ...
        let capability = session
            ...
            .ok_or_else(|| {
                SecurityError::CapabilityNotGranted(format!(
                    "No capability for {:?}",
        ...
        // Check constraints
        self.validate_constraints(capability, resource, session)
            .await?;
    ...
    fn matches_resource(
        &self,
        capability_resource: &ResourceType,
        ...
        match (capability_resource, &request.resource_type) {
            ...
            (ResourceType::Network { hosts, ports, .. }, ResourceType::Network { .. }) => {
                ...
                hosts.iter().any(|host| request.target.contains(host))
                    || ports
                        .iter()
                        .any(|port| request.target.contains(&port.to_string()))
            }
            (ResourceType::Process { commands, .. }, ResourceType::Process { .. }) => {
                commands.iter().any(|cmd| request.target.starts_with(cmd))
            }
            _ => {
                std::mem::discriminant(capability_resource)
                    == std::mem::discriminant(&request.resource_type)
            }
        }
    }
            ...
            match constraint {
                Constraint::MaxFileSize(max_size) => {
                    if let Some(size) = resource.size {
                        if size > *max_size {
                            return Err(SecurityError::ConstraintViolation(format!(
                ...
                Constraint::MaxMemoryUsage(max_memory) => {
                    if session.resource_usage.memory_used > *max_memory {
                        return Err(SecurityError::ConstraintViolation(format!(
                            ...
                            session.resource_usage.memory_used, max_memory
                        ))
                        .into());
                    }
                }
        ...
        for condition in conditions {
            match condition {
                ...
                Condition::TimeOfDay { start, end } => {
                    ...
                    if now < *start || now > *end {
                        unmet_conditions
                            .push(format!("Time must be between {} and {}", start, end));
                    }
                }
                ...
                Condition::Custom { key, value } => {
                    if let Some(actual_value) = session.metadata.get(key) {
                        if actual_value != value {
                            unmet_conditions
                                .push(format!("Custom condition {}={} required", key, value));
                        }
                    } else {
                        unmet_conditions
                            .push(format!("Custom condition {}={} required", key, value));
    ...
    async fn check_rate_limit(
        ...
        if !limiter.allow_request() {
            return Err(
                SecurityError::ConstraintViolation("Rate limit exceeded".to_string()).into(),
            );
        }
...
mod tests {
    ...
    async fn test_session_creation() {
        ...
        let session_id = manager
            .create_session("default", Some("test_user".to_string()), HashMap::new())
            .await
            .unwrap();
