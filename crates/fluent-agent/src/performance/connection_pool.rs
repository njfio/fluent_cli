use super::{ConnectionPoolConfig, utils::PerformanceCounter};
use anyhow::Result;
...
impl Manager for HttpClientManager {
    ...
    async fn recycle(&self, client: &mut Client, _metrics: &deadpool::managed::Metrics) -> Result<(), deadpool::managed::RecycleError<Self::Error>> {
...
mod tests {
    ...
    async fn test_connection_pool_manager() {
        ...
        let pool_result = manager.get_or_create_pool(
            "test_pool",
            "http://localhost:8080".to_string(),
            HashMap::new(),
            None,
