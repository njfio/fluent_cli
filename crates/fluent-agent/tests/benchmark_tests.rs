...
use fluent_agent::{
    ...
    performance::utils::{MemoryTracker, PerformanceCounter},
};
...
async fn benchmark_memory_operations() -> Result<()> {
    ...
    for (size_name, content_size, num_items) in test_cases {
        println!(
            "\nTesting {} memories ({} bytes each, {} items):",
            size_name, content_size, num_items
        );
        ...
        for i in 0..num_items {
            ...
            let memory = MemoryItem {
                ...
                metadata: {
                    ...
                    meta.insert(
                        "size".to_string(),
                        serde_json::Value::String(size_name.to_string()),
                    );
        ...
        println!(
            "  Store: {} items in {:?} (avg: {:?}, {:.2} items/sec)",
            num_items,
            store_time,
            store_stats.average_duration,
            num_items as f64 / store_time.as_secs_f64()
        );
        println!(
            "  Retrieve: {} items in {:?} ({:.2} items/sec)",
            results.len(),
            retrieve_time,
        ...
        assert!(
            store_time < Duration::from_secs(60),
            "{} store benchmark too slow",
            size_name
        );
        assert!(
            retrieve_time < Duration::from_secs(10),
            "{} retrieve benchmark too slow",
            size_name
        );
        assert!(
            store_stats.error_rate < 0.05,
            "{} too many store errors",
            size_name
        );
    }
...
async fn benchmark_concurrent_load() -> Result<()> {
    ...
    for concurrency in concurrency_levels {
        ...
        assert!(
            ...
            "Concurrency {} too slow",
            concurrency
        );
        assert!(
            total_success > total_operations * 8 / 10,
            "Concurrency {} too many failures",
            concurrency
        );
    }
...
async fn benchmark_query_patterns() -> Result<()> {
    ...
    println!(
        "Populating {} memories for query benchmarking...",
        num_memories
    );
    ...
    for i in 0..num_memories {
        let memory = MemoryItem {
            ...
            content: format!(
                "Query benchmark memory {} about topic {} in category {}",
                i,
                i % 50,
                i % 10
            ),
            metadata: {
                ...
                meta.insert(
                    "topic".to_string(),
                    serde_json::Value::Number((i % 50).into()),
                );
                meta.insert(
                    "category".to_string(),
    ...
    for (pattern_name, num_items, content_size) in patterns {
        ...
        for i in 0..num_items {
            ...
            if i % (num_items / 10).max(1) == 0 {
                ...
                println!(
                    "    Progress: {}/{}, Memory: {} bytes",
                    i, num_items, current_usage
                );
            }
        }
        ...
        let mem_delta = end_usage.saturating_sub(start_usage);

        println!("  Pattern completed in {:?}", pattern_time);
        println!(
            "  Memory usage: {} -> {} bytes (peak: {})",
            start_usage, end_usage, peak_usage
        );
        println!(
            "  Memory per item: {} bytes",
            mem_delta / (num_items as u64)
        );
        println!(
            "  Items per second: {:.2}",
            num_items as f64 / pattern_time.as_secs_f64()
        );

        // Performance assertions
        assert!(
            pattern_time < Duration::from_secs(300),
            "Pattern '{}' too slow",
            pattern_name
        );
    }
