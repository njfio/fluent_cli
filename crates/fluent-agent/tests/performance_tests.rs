...
use fluent_agent::{
    ...
    performance::utils::{MemoryTracker, PerformanceCounter, ResourceLimiter},
};
...
async fn test_memory_store_performance() -> Result<()> {
    ...
    println!(
        "Similarity search completed in {:?}, found {} matches",
        similarity_time,
        similar.len()
    );
    ...
    assert!(
        total_store_time < Duration::from_secs(30),
        "Store operations too slow"
    );
    assert!(
    ...
    assert!(
        similarity_time < Duration::from_secs(10),
...
async fn test_orchestrator_performance() -> Result<()> {
    ...
    for i in 0..num_goals {
        ...
        let _state = orchestrator
            .process_goal(goal.clone(), context.clone())
            .await?;
        ...
        println!(
            "Goal {} processed in {:?}, decomposed in {:?}",
            i, process_time, decomp_time
        );
    }
...
async fn test_mcp_tool_registry_performance() -> Result<()> {
    ...
    // Performance assertions
    assert!(
        registration_time < Duration::from_secs(10),
...
async fn test_performance_counter() -> Result<()> {
    ...
    for i in 0..1000 {
        ...
        if i % 100 == 0 {
            ...
            println!(
                "Operations: {}, Avg time: {:?}",
                stats.total_operations, stats.average_duration
            );
        }
    }
    ...
    println!(
        "Final stats - Operations: {}, Total time: {:?}, Avg: {:?}",
        final_stats.total_operations, final_stats.total_duration, final_stats.average_duration
    );
...
async fn test_large_data_handling() -> Result<()> {
    ...
    assert!(
        store_time < Duration::from_secs(5),
        "Large data storage too slow"
    );
    assert!(
        retrieve_time < Duration::from_secs(2),
        "Large data retrieval too slow"
    );
