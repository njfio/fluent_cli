...
use fluent_agent::{
    ...
    reflection::{ReflectionConfig, ReflectionEngine},
    task::TaskBuilder,
    tools::ToolRegistry,
};
...
async fn test_orchestrator_error_handling() -> Result<()> {
    ...
    let result = orchestrator
        .process_goal(valid_goal.clone(), empty_context)
        .await;
    ...
    let decomposition_result = orchestrator
        .decompose_goal(&complex_goal, &minimal_context)
        .await;
...
async fn test_mcp_error_scenarios() -> Result<()> {
    ...
    let registry = McpToolRegistry::new(Arc::new(ToolRegistry::new()));
