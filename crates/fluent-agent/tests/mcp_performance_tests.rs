...
use fluent_agent::{
    ...
    mcp_tool_registry::{McpToolDefinition, McpToolRegistry},
...
async fn test_mcp_tool_registry_performance() -> Result<()> {
    ...
    println!(
        "  Registrations per second: {:.2}",
        num_tools as f64 / total_registration_time.as_secs_f64()
    );
    ...
    assert!(
        total_registration_time < Duration::from_secs(30),
        "Tool registration too slow"
    );
    assert!(list_time < Duration::from_secs(2), "Tool listing too slow");
    assert!(
        avg_retrieval_time < Duration::from_millis(10),
        "Tool retrieval too slow"
    );
...
async fn test_mcp_client_connection_performance() -> Result<()> {
    ...
    assert!(
        ...
        "MCP client operations too slow"
    );
...
async fn test_concurrent_mcp_operations() -> Result<()> {
    ...
    println!(
        "Testing concurrent MCP operations with {} tasks...",
        num_concurrent
    );
            ...
            for op_id in 0..operations_per_task {
                ...
                match task_registry.register_tool(tool_def).await {
                    Ok(_) => {
                        ...
                        let _tool = task_registry
                            .get_tool(&format!("concurrent_tool_{}_{}", task_id, op_id))
                            .await;
                    }
                    Err(_) => task_errors += 1,
                }
    ...
    assert!(
        ...
        "Concurrent MCP operations too slow"
    );
    assert!(
...
async fn test_mcp_resource_limiting() -> Result<()> {
    ...
    println!(
        "Testing MCP resource limiting with {} operations...",
        num_operations
    );
...
async fn test_mcp_tool_search_performance() -> Result<()> {
    ...
    println!(
        "Registering {} tools across {} categories...",
        tools_per_category * categories.len(),
        categories.len()
    );
...
async fn test_mcp_memory_usage() -> Result<()> {
    ...
    println!(
        "Testing MCP memory usage with {} large tools...",
        num_large_tools
    );
    ...
    // Performance assertions
    assert!(
        list_time < Duration::from_secs(5),
        "Large tool listing too slow"
    );
