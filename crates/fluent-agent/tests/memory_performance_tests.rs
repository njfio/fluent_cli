...
async fn test_memory_store_throughput() -> Result<()> {
    ...
    for i in 0..num_writes {
        ...
        let memory = MemoryItem {
            ...
            metadata: {
                ...
                meta.insert(
                    "batch".to_string(),
                    serde_json::Value::String(format!("batch_{}", i / 100)),
                );
                meta
            },
    ...
    println!(
        "  Writes per second: {:.2}",
        num_writes as f64 / total_time.as_secs_f64()
    );
...
async fn test_memory_query_performance() -> Result<()> {
    ...
    for i in 0..num_memories {
        let memory = MemoryItem {
            ...
            content: format!(
                "Query test memory {} with content about topic {}",
                i,
                i % 20
            ),
...
async fn test_memory_stress_test() -> Result<()> {
    ...
    println!(
        "Starting memory stress test with {} large memories...",
        num_large_memories
    );
    ...
    for i in 0..num_large_memories {
        let memory = MemoryItem {
            ...
            metadata: {
                ...
                for j in 0..50 {
                    meta.insert(
                        format!("key_{}", j),
                        serde_json::Value::String(format!("value_{}", j)),
                    );
                }
                meta
            },
        ...
        if i % 10 == 0 {
            ...
            println!(
                "  Stored {} memories, memory usage: {} bytes",
                i + 1,
                current_usage
            );
        }
    ...
    println!(
        "  Stored {} large memories in {:?}",
        num_large_memories, store_time
    );
    println!(
    ...
    // Performance assertions
    assert!(
        store_time < Duration::from_secs(120),
        "Stress test storage too slow"
    );
    assert!(
        retrieval_time < Duration::from_secs(10),
        "Stress test retrieval too slow"
    );
