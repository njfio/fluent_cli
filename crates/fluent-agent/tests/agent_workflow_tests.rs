...
use fluent_agent::{
    ...
    context::{ContextVariable, ExecutionContext},
    goal::{Goal, GoalBuilder, GoalComplexity, GoalStatus},
    memory::{AsyncSqliteMemoryStore, LongTermMemory, MemoryConfig, ShortTermMemory},
    orchestrator::{AgentOrchestrator, AgentState, OrchestrationMetrics},
    reasoning::{ReasoningEngine, ReasoningPrompts},
    reflection::{ReflectionConfig, ReflectionEngine, ReflectionType},
    task::{Task, TaskBuilder, TaskComplexity, TaskStatus},
    tools::ToolRegistry,
};
use std::sync::Arc;
...
async fn test_complete_agent_workflow() -> Result<()> {
    // Setup components
    let memory_system =
        Arc::new(AsyncSqliteMemoryStore::new(":memory:")?) as Arc<dyn LongTermMemory>;
    ...
    context.add_variable(
        "target_file",
        ContextVariable::String("hello.txt".to_string()),
    );
    context.add_variable(
        "content",
        ContextVariable::String("Hello, World!".to_string()),
    );
...
async fn test_reflection_integration() -> Result<()> {
    ...
    let reflection_engine =
        ReflectionEngine::new(ReflectionConfig::default(), memory_system.clone());

    // Create test execution context
    let mut context = ExecutionContext::new("reflection_test".to_string());
    context.add_variable(
        "test_result",
        ContextVariable::String("success".to_string()),
    );
    ...
    let reflection_type = reflection_engine
        .determine_reflection_type(&context)
        .await?;
...
async fn test_context_management() -> Result<()> {
    ...
    context.add_variable(
        "string_var",
        ContextVariable::String("test_value".to_string()),
    );
...
async fn test_error_handling_in_workflow() -> Result<()> {
    ...
    let invalid_goal_result = GoalBuilder::new().with_description("").build();
