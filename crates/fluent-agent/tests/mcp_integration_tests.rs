...
use fluent_agent::{
    ...
    mcp_client::{ClientInfo, McpClient},
...
use fluent_mcp::model::{CallToolRequest, Content, ServerInfo, Tool};
use std::sync::Arc;
use tokio;
...
async fn test_mcp_concurrent_operations() -> Result<()> {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            registry_clone
                .get_tool(&format!("concurrent_tool_{}", i))
                .await
        });
...
async fn test_mcp_resource_uri_parsing() -> Result<()> {
    ...
    for uri in test_uris {
        ...
        if uri.starts_with("memory://") {
            ...
            assert!(
                result.is_ok(),
                "Expected memory URI {} to succeed, got {:?}",
                uri,
                result
            );
        }
