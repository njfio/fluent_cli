...
fn create_mcp_matches(subcommand: &str, args: Vec<(&str, &str)>) -> ArgMatches {
    ...
    let cmd = Command::new("mcp")
        ...
        .subcommand(Command::new("disconnect").arg(Arg::new("name").long("name")))
        .subcommand(
            Command::new("tools")
...
async fn test_mcp_agent_legacy_command() -> Result<()> {
    ...
    let matches = create_mcp_matches("agent", vec![("engine", "openai"), ("task", "test task")]);
...
async fn test_mcp_agent_invalid_engine() -> Result<()> {
    ...
    let matches = create_mcp_matches(
        "agent",
        vec![("engine", "invalid_engine"), ("task", "test task")],
    );
...
async fn test_mcp_cli_integration() -> Result<()> {
    ...
    for subcommand in subcommands {
        ...
        let matches = match subcommand {
            "connect" => {
                create_mcp_matches(subcommand, vec![("name", "test"), ("command", "test")])
            }
