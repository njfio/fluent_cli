...
impl ConfigCli {
    ...
    pub async fn run() -> Result<()> {
        ...
        match cli.command {
            Commands::Create { engine_type, name, description } => {
                Self::create_config(&manager, &engine_type, &name, description.as_deref()).await
            }
            Commands::List => {
                Self::list_configs(&manager).await
            }
            Commands::Show { name } => {
                Self::show_config(&manager, &name).await
            }
            Commands::Validate { name } => {
                Self::validate_config(&manager, &name).await
            }
            Commands::Update { name, set } => {
                Self::update_config(&manager, &name, set).await
            }
            ...
            Commands::Copy { from, to } => {
                Self::copy_config(&manager, &from, &to).await
            }
            Commands::Delete { name, force } => {
                Self::delete_config(&manager, &name, force).await
            }
        }
    }
    ...
    async fn create_config(
        ...
        println!("✅ Created configuration '{}' for engine type '{}'", name, engine_type);
        
        // Show next steps
        println!("\n📝 Next steps:");
        println!("  1. Add required parameters: fluent-config update {} --set bearer_token=YOUR_TOKEN", name);
        println!("  2. Validate configuration: fluent-config validate {}", name);
    ...
    async fn list_configs(manager: &ConfigManager) -> Result<()> {
        ...
        for config_name in configs {
            match manager.get_metadata(&config_name).await {
                Ok(metadata) => {
                    println!("  • {} ({})", config_name, metadata.description.unwrap_or_else(|| "No description".to_string()));
                    println!("    Version: {}, Updated: {}", metadata.version, metadata.updated_at);
                }
                Err(_) => {
    ...
    async fn update_config(manager: &ConfigManager, name: &str, updates: Vec<String>) -> Result<()> {
        let mut parameter_updates = HashMap::new();
        
        for update in updates {
            let parts: Vec<&str> = update.splitn(2, '=').collect();
            if parts.len() != 2 {
                return Err(anyhow!("Invalid update format: '{}'. Use KEY=VALUE", update));
            }
    ...
    async fn add_environment(
        ...
        for override_str in overrides {
            ...
            if parts.len() != 2 {
                return Err(anyhow!("Invalid override format: '{}'. Use KEY=VALUE", override_str));
            }
        ...
        enhanced_config.environments.insert(env.to_string(), env_override);
        manager.save_config(name, &enhanced_config).await?;
        
        println!("✅ Added environment '{}' overrides to configuration '{}'", env, name);
        
        Ok(())
    }

    async fn export_config(manager: &ConfigManager, name: &str, output: Option<&std::path::Path>) -> Result<()> {
        let config_path = manager.config_dir.join(format!("{}.json", name));
        let content = tokio::fs::read_to_string(&config_path).await?;
    ...
    async fn import_config(manager: &ConfigManager, input: &std::path::Path, name: Option<&str>) -> Result<()> {
        ...
        println!("✅ Imported configuration '{}' from {}", config_name, input.display());
...
mod tests {
    ...
    fn test_parse_value() {
        ...
        assert_eq!(ConfigCli::parse_value("42").unwrap(), Value::Number(serde_json::Number::from(42)));
        assert_eq!(ConfigCli::parse_value("3.14").unwrap(), Value::Number(serde_json::Number::from_f64(3.14).unwrap()));
        assert_eq!(ConfigCli::parse_value("hello").unwrap(), Value::String("hello".to_string()));
    }
}
