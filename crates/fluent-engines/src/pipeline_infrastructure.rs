use crate::modular_pipeline_executor::{VariableExpander, StateStore, EventListener, ExecutionContext, ExecutionEvent};
        ...
        match &event.event_type {
            ...
            crate::modular_pipeline_executor::EventType::StepStarted => {
                if let Some(step_name) = &event.step_name {
                    println!("[{}] 🔄 Step started: {} ({})", timestamp, step_name, event.run_id);
                }
            }
            crate::modular_pipeline_executor::EventType::StepCompleted => {
                if let Some(step_name) = &event.step_name {
                    println!("[{}] ✅ Step completed: {} ({})", timestamp, step_name, event.run_id);
                }
            }
            crate::modular_pipeline_executor::EventType::StepFailed => {
                if let Some(step_name) = &event.step_name {
                    println!("[{}] ❌ Step failed: {} ({})", timestamp, step_name, event.run_id);
                }
            }
            crate::modular_pipeline_executor::EventType::StepRetrying => {
                if let Some(step_name) = &event.step_name {
                    println!("[{}] 🔄 Step retrying: {} ({})", timestamp, step_name, event.run_id);
                }
            }
...
impl PipelineExecutorBuilder {
    ...
    pub fn with_file_logging(mut self, log_file: PathBuf) -> Self {
        self.event_listeners.push(Arc::new(FileEventListener::new(log_file)));
        self
    }
    ...
    pub fn build(self) -> Result<crate::modular_pipeline_executor::ModularPipelineExecutor> {
        ...
        let variable_expander = self.variable_expander
        ...
        // Add step executors
        executor.register_step_executor(Arc::new(crate::pipeline_step_executors::CommandStepExecutor));
        executor.register_step_executor(Arc::new(crate::pipeline_step_executors::HttpStepExecutor::new()));
        executor.register_step_executor(Arc::new(crate::pipeline_step_executors::FileStepExecutor));
        executor.register_step_executor(Arc::new(crate::pipeline_step_executors::ConditionStepExecutor));
...
mod tests {
    ...
    async fn test_condition_evaluation() {
        ...
        assert!(expander.evaluate_condition("true", &variables).await.unwrap());
        assert!(!expander.evaluate_condition("false", &variables).await.unwrap());
        assert!(expander.evaluate_condition("${status} == success", &variables).await.unwrap());
        assert!(!expander.evaluate_condition("${status} == failure", &variables).await.unwrap());
        assert!(expander.evaluate_condition("${count} > 3", &variables).await.unwrap());
        assert!(!expander.evaluate_condition("${count} < 3", &variables).await.unwrap());
    }
