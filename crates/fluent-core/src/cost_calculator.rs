...
impl CostCalculator {
    ...
    pub fn calculate_cost(&mut self, engine: &str, model: &str, usage: &Usage) -> Result<Cost> {
        debug!("Calculating cost for engine: {}, model: {}, usage: {:?}", engine, model, usage);
        ...
        let completion_cost = (usage.completion_tokens as f64 / 1_000_000.0) * pricing.completion_rate;
        ...
        // Log for audit trail
        debug!("Cost calculated: prompt=${:.6}, completion=${:.6}, total=${:.6}", 
               prompt_cost, completion_cost, total_cost);
        
        if total_cost > self.limits.warn_threshold {
            warn!("High cost detected: ${:.6} for engine: {}, model: {}", 
    ...
    fn find_pricing_model(&self, engine: &str, model: &str) -> Result<&PricingModel> {
        ...
        for (key, pricing) in &self.pricing_models {
            if key.starts_with(&format!("{}:", engine)) && model.contains(&pricing.model) {
                debug!("Using partial match for pricing: {} -> {}", model, pricing.model);
                return Ok(pricing);
            }
        }
        
        Err(anyhow!("No pricing model found for engine: {}, model: {}", engine, model))
    }
    ...
    fn validate_cost(&self, cost: &Cost) -> Result<()> {
        ...
        if cost.total_cost > self.limits.max_single_request {
            return Err(anyhow!("Cost ${:.6} exceeds maximum single request limit of ${:.2}", 
                              cost.total_cost, self.limits.max_single_request));
        }
        
        // Check daily limit
        if self.daily_total + cost.total_cost > self.limits.max_daily_total {
            return Err(anyhow!("Cost ${:.6} would exceed daily limit of ${:.2} (current: ${:.2})", 
                              cost.total_cost, self.limits.max_daily_total, self.daily_total));
        }
...
mod tests {
    ...
    fn test_calculate_openai_cost() {
        ...
        let cost = calculator.calculate_cost("openai", "gpt-3.5-turbo", &usage).unwrap();
    ...
    fn test_calculate_anthropic_cost() {
        ...
        let cost = calculator.calculate_cost("anthropic", "claude-3-sonnet", &usage).unwrap();
    ...
    fn test_zero_tokens() {
        ...
        let cost = calculator.calculate_cost("openai", "gpt-4", &usage).unwrap();
    ...
    fn test_reset_daily_total() {
        ...
        calculator.calculate_cost("openai", "gpt-3.5-turbo", &usage).unwrap();
